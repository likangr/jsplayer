...
int __JS_ANDROID_SDK_VERSION__ = -1;

int js_get_sdk_version() {
    if (__JS_ANDROID_SDK_VERSION__ != -1) {
        return __JS_ANDROID_SDK_VERSION__;
    }

    char versions[8];
    if (__system_property_get("ro.build.version.sdk", versions) > 0) {
        __JS_ANDROID_SDK_VERSION__ = atoi(versions);
        return __JS_ANDROID_SDK_VERSION__;
    } else {
        return -1;
    }
...
int js_jni_exception_get_summary(JNIEnv *env, jthrowable exception, char **error, void *log_ctx) {
    ...
    if (string) {
        name = js_jni_jstring_to_utf_chars(env, string, log_ctx);
    ...
    if (string) {
        message = js_jni_jstring_to_utf_chars(env, string, log_ctx);
        (*env)->DeleteLocalRef(env, string);
        string = NULL;
    }
...
int js_jni_exception_check(JNIEnv *env, int log, void *log_ctx) {
    ...
    if ((ret = js_jni_exception_get_summary(env, exception, &message, log_ctx)) < 0) {
...
int js_jni_init_jfields(JNIEnv *env, void *jfields, const struct JSJniField *jfields_mapping,
    ...
    for (i = 0; jfields_mapping[i].name; i++) {
        ...
        enum JSJniFieldType type = jfields_mapping[i].type;

        if (type == JS_JNI_CLASS) {
            jclass clazz;

            last_clazz = NULL;

            clazz = (*env)->FindClass(env, jfields_mapping[i].name);
            if ((ret = js_jni_exception_check(env, mandatory, log_ctx)) < 0 && mandatory) {
                goto done;
            }
    ...
    if (ret < 0) {
        ...
        js_jni_reset_jfields(env, jfields, jfields_mapping, global, log_ctx);
    }
